...
def TrainConvNet():
    ...
    with graph.as_default():
        ...
        lengths = tf.placeholder(tf.float32, shape=(None, 1, 5))
        ...
        w_s_1 = weight_layer("z_s_1", [hl, num_labels])
        b_s_1 = bias_variable("b_s_1", [num_labels])
        z_s_1 = tf.matmul(h_fc, w_s_1) + b_s_1
        
        w_s_2 = weight_layer("z_s_2", [hl, num_labels])
        b_s_2 = bias_variable("b_s_2", [num_labels])
        z_s_2 = tf.matmul(h_fc, w_s_2) + b_s_2
        
        w_s_3 = weight_layer("z_s_3", [hl, num_labels])
        b_s_3 = bias_variable("b_s_3", [num_labels])
        z_s_3 = tf.matmul(h_fc, w_s_3) + b_s_3

        w_s_4 = weight_layer("z_s_4", [hl, num_labels])
        b_s_4 = bias_variable("b_s_4", [num_labels])
        z_s_4 = tf.matmul(h_fc, w_s_4) + b_s_4

        w_s_5 = weight_layer("z_s_5", [hl, num_labels])
        b_s_5 = bias_variable("b_s_5", [num_labels])
        ...
        five_gate = tf.minimum(lengths[:,0,4], [1.0])
        four_gate = tf.maximum(lengths[:,0,3], five_gate)
        three_gate = tf.maximum(lengths[:,0,2], four_gate)
        two_gate = tf.maximum(lengths[:,0,1], three_gate)
        one_gate = tf.maximum(lengths[:,0,0], two_gate)
